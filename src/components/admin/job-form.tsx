...
export function JobForm({ job }: { job?: JobData }) {
  ...
  const inputCls =
    "w-full rounded-xl border border-white/10 bg-white/[0.03] px-3.5 py-3 text-sm sm:text-xs text-white placeholder:text-neutral-500 outline-none transition focus:border-amp-blue focus:bg-black focus:ring-1 focus:ring-amp-blue/50";
    ...
    <form onSubmit={onSubmit} className="space-y-5 sm:space-y-6">
      ...
      <div>
        ...
        <input
          ...
          className={inputCls}
          placeholder="Contoh: Senior Full-Stack Engineer / Product Designer"
        />
      </div>
      ...
      <div>
        <label htmlFor="slug" className={labelCls}>
          ...
          <span>Slug URL</span>
          <span className="text-neutral-500 font-normal">(opsional: otomatis dari judul)</span>
        </label>
        <input
          id="slug"
          name="slug"
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:gap-5">
        ...
        <div>
          ...
          >
            <option value="fulltime" className="bg-[#0e0e0e]">Full-time</option>
            <option value="contract" className="bg-[#0e0e0e]">Contract</option>
            <option value="freelance" className="bg-[#0e0e0e]">Freelance</option>
            <option value="internship" className="bg-[#0e0e0e]">Internship</option>
          </select>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className={inputCls}
          placeholder="• Minimal 2+ tahun pengalaman Next.js & React&#10;• Memahami AI integration & Clean architecture..."
        />
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="bg-btn-gradient flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl px-6 py-3.5 text-xs font-bold text-white shadow-lg shadow-blue-500/20 transition-all hover:brightness-110 active:scale-95 disabled:opacity-50"
      >
        {loading ? (
          <>
